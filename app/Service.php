...
class Service
{
    ...
    private $cache = [];
    /**
     * @var array $instances Stores any instances marked as 'shared' so get() can return the same instance
     */
    private $instances = [];
    /**
    ...
    private function getClosure($name, array $rule) {
        ...
        if (!empty($rule['shared'])) $closure = function (array $args, array $share) use ($class, $name, $constructor, $params) {
            // Shared instance: get the class without calling the constructor (and write to \$name and $name, see issue #68)
    ...
    private function expand($param, array $share = [], $createFromString = false) {
        if (is_array($param) && isset($param['instance'])) {
            ...
            else return $this->get($param['instance'], array_merge($args, $share));
        }
    ...
    private function getParams(\ReflectionMethod $method, array $rule) {
        ...
        return function (array $args, array $share = []) use ($paramInfo, $rule) {
            // Now merge all the possible parameters: user-defined in the rule via constructParams, shared instances and the $args argument from $dice->get();
